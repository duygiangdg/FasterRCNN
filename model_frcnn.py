...
def fastrcnn_outputs(feature, num_classes):
    ...
    angle_regression = FullyConnected('angle', feature, num_classes,
        kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    angle_regression = tf.reshape(angle_regression, (-1, 1), name='output_angle')
...
def fastrcnn_losses(labels, label_logits, fg_boxes, fg_box_logits, angles, angle_logits):
    ...
    angle_loss = tf.losses.huber_loss(tf.reshape(angles, (-1, 1)), angle_logits, reduction=tf.losses.Reduction.SUM)
